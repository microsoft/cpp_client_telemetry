...
namespace
{
    ...
    public:
        ...
        void SetQueueValidation(std::function<void(evt_task*)> fn) { m_validateQueueFn = fn; }
    ...
    private:
        ...
        std::function<void(evt_task*)> m_validateQueueFn;
...
void EVTSDK_LIBABI_CDECL OnTaskDispatcherQueue(evt_task* task, evt_task_callback_fn callback)
{
...
void CheckTaskTypeNameIsExpectedOrEmptyIfRTTIIsEnabled(evt_task* task) noexcept
{
...
TEST(TaskDispatcherCAPITests, Execute)
{
    ...
    testHelper->SetQueueValidation([&wasQueued](evt_task* task) {
        wasQueued = true;
...
TEST(TaskDispatcherCAPITests, Cancel)
{
    ...
    string taskIdStr;
    bool wasQueued = false;
    testHelper->SetQueueValidation([&wasQueued, &taskIdStr](evt_task* task) {
        wasQueued = true;
        taskIdStr = task->id;
        EXPECT_NE(task->delayMs, 0);
        CheckTaskTypeNameIsExpectedOrEmptyIfRTTIIsEnabled(task);

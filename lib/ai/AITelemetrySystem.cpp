...
/// <param name="bandwidthController">The bandwidth controller.</param>
    AITelemetrySystem::AITelemetrySystem(
        ...
        tpm(*this, taskDispatcher, bandwidthController)
    {
        ...
        onStop = [this](void)
        {
            ...
            if ((timeoutInSec > 0) && (!tpm.isPaused()))
            {
                ...
                // TODO: GetRecordCount throws an error
        ...
        this->sending >> aiJsonSerializer.serialize >> this->incomingEventPrepared;
...
//        storage.retrievalFinished >> packager.finalizePackage;
    ...
    void AITelemetrySystem::handleIncomingEventPrepared(IncomingEventContextPtr const& event)
    {
        ...
        std::string const& configToken = m_config[CFG_STR_PRIMARY_TOKEN];
        if (event->record.tenantToken != configToken) {
            LOG_INFO("Event %s/%s dropped because only primary token supported",
                     tenantTokenToId(event->record.tenantToken).c_str(), event->source->baseType.c_str());
            return;
        }
    ...
    void AITelemetrySystem::handleFlushTaskDispatcher()
    {
        signalDone();
    }

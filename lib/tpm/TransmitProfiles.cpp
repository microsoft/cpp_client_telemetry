...
namespace ARIASDK_NS_BEGIN {
            ...
            void TransmitProfiles::dump() {
                ...
                for (auto &kv : profiles) {
                    ...
                    ARIASDK_LOG_DETAIL("name=%s", profile.name.c_str());
                    size_t i = 0;
                    for (auto &rule : profile.rules) {
                        ARIASDK_LOG_DETAIL("[%d] netCost=%2d, powState=%2d, timers=[%3d,%3d,%3d]",
                ...
                do {
                    ...
                    if (currTimerValue > 0) {
                        if (currTimerValue < prevTimerValue) {
                            ...
                            ARIASDK_LOG_WARNING("Low-pri timer can't be lower than high-pri: timer[%d] adjusted %d=>%d", i, rule.timers[i], currTimerValue);
                        ...
                        if (prevTimerValue > 0) {
                            ...
                            if (mod != 0) {
                                ...
                                ARIASDK_LOG_WARNING("Low-pri timer must be multiple of high-pri: timer[%d] adjusted %d=>%d", i, rule.timers[i], currTimerValue);
                                rule.timers[i] = currTimerValue;
                                isAutoCorrected = true;
                            }
                        }
                            ...
                            currTimerValue = prevTimerValue;
                            ARIASDK_LOG_WARNING("Low-pri timer can't be on if high-pri is off: timer[%d] adjusted %d=>%d", i, rule.timers[i], currTimerValue);
                            rule.timers[i] = currTimerValue;
                            isAutoCorrected = true;
                        }
                    }
                    prevTimerValue = currTimerValue;
                    ARIASDK_LOG_DETAIL("timers[%d]=%d", i, currTimerValue);
                } while (i != 0);
            ...
            size_t TransmitProfiles::parse(const std::string& profiles_json)
			{
                ...
                numProfilesParsed = newProfiles.size();
                {
                    ...
                    if (it == profiles.end()) {
                        ...
                        ARIASDK_LOG_DETAIL("Switched to profile %s", currProfileName.c_str() );
                    }
                ...
                ARIASDK_LOG_INFO("JSON parsing completed successfully [%d]", numProfilesParsed);
...
parsing_failed:
            ...
            bool TransmitProfiles::load(std::string profiles_json) {
                ...
                ARIASDK_LOG_DETAIL("Loading custom profiles...");
            ...
            bool TransmitProfiles::setProfile(const std::string& profileName) {
...

                {
                    ...
                    if (it != profiles.end()) {
                        ...
                        ARIASDK_LOG_INFO("selected profile %s ...", profileName.c_str());
                        result = true;
                    }
                    else {
                        ARIASDK_LOG_WARNING("profile %s not found!", profileName.c_str());
                        currProfileName = DEFAULT_PROFILE;
                        ARIASDK_LOG_WARNING("selected profile %s instead", currProfileName.c_str());
                    }
                }
                updateStates(currNetCost, currPowState);
                return result;
            ...
            void TransmitProfiles::getTimers(std::vector<int>& out) {
                {                    
                    ...
                    if (it == profiles.end()) {
                        ...
                        ARIASDK_LOG_WARNING("No active profile found, disabling all transmission timers.");
                        return;
                    }
            ...
            void TransmitProfiles::onTimersUpdated() {
                ...
                if (it != profiles.end()) {
                    ...
                    ARIASDK_LOG_INFO("timers=[%3d,%3d,%3d]",
                        rule.timers[0],
                        rule.timers[1],
                        rule.timers[2]);
#endif

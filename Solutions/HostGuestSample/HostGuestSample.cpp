    ...
    virtual void OnDebugEvent(DebugEvent &evt)
    {
        ...
        case EVT_STORAGE_FULL:
            ...
            if (evt.param1 >= 75) {
                ...
                //std::thread([]() { host->UploadNow(); }).detach();
            }
            break;
...
ILogConfiguration& configuration = LogManagerProvider::CreateLogConfiguration();
...
[{
    ...
    host->SetContext("GLOBAL_context", gc_value, PiiKind_MailSubject);
...
void run(ILogger* logger, int maxStressRuns) {
    {
        ...
        for (int stressRuns = 0; stressRuns < maxStressRuns; stressRuns++)
        {
...

            {
                ...
                ILogger *loggerl = host->GetLogger(cTenantToken);
...
{//
    ...
    */
    ...
    //loggers["logger.noparam"] = host->GetLogger();
    //loggers["logger.blank"] = host->GetLogger("");
    //loggers["logger.blank.s1"] = host->GetLogger("", "s1");
    //loggers["logger.blank.s2"] = host->GetLogger("", "s2");
    //loggers["logger.invalid"] = host->GetLogger("s3", "12345");
    //loggers["logger.invalid"] = host->GetLogger("12345", "s3");
    loggers["logger.primary"] = host->Initialize(TOKEN, configuration);
    loggers["logger.t1s1"] = host->GetLogger(TOKEN, "s1");
    loggers["logger.t1s2"] = host->GetLogger(TOKEN, "s2");
    loggers["logger.t2s1"] = host->GetLogger(TOKEN2, "s1");
    loggers["logger.t2s2"] = host->GetLogger(TOKEN2, "s2");
    for (auto &kv : loggers)
    {

...
MyDebugEventListener debugListener;
...
EventProperties CreateSampleEvent(const char *name, EventPriority prio) {
	...
	EventProperties props(name,
	{
		...
		{ "dblKey",   3.14 },
		{ "boolKey",  false },
		{ "guidKey0", GUID_t("11100000-1111-0000-0000-000000000000") },
...
void addEventListener(DebugEventListener& listener)
{
...
void removeEventListener(DebugEventListener& listener)
{
    ...
    LogManager::RemoveEventListener(DebugEventType::EVT_UNKNOWN, listener);
}
...
ILogger* init() {
    ...
    if (IntptrSize > 4)
    {
        configuration.SetProperty(CFG_STR_CACHE_FILE_PATH, "C:\\temp\\x64\\offlinestorage.db"); //":memory:"; //"offlinestorage.db";
    }
    else
    {
        configuration.SetProperty(CFG_STR_CACHE_FILE_PATH, "C:\\temp\\offlinestorage.db"); //":memory:"; //"offlinestorage.db";
    }
    xPtr = xPtr;
...
[{
    ...
    addEventListener(debugListener);
...
void done() {
    ...
    removeEventListener(debugListener);
}

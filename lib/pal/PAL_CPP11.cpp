    ...
    namespace PAL {
        ...
        void dumpRefCounted()
        {
            ...
            for (auto &kv : refCountedTracker)
            {
                ...
                replace(key, "Microsoft::Applications::Telemetry::", "MAT::");
                replace(key, "Microsoft::Applications::Telemetry::PAL::", "PAL::");
...
#define     gettid()       std::this_thread::get_id()
        ...
        public:
            ...
            WorkerThread()
            {
                ...
                ARIASDK_LOG_INFO("Started new thread %u", m_hThread.get_id());
            }
            ...
            {
                ARIASDK_LOG_INFO("queue item=%p, type=%u", item.get(), item.get()->type);
        ...
        protected:
            static void threadFunc(void* lpThreadParameter)
            {
                ...
                ARIASDK_LOG_INFO("Running thread %u", std::this_thread::get_id());
                ...
                for (;;) {
                    ...
                    ARIASDK_LOG_DETAIL("Execute item=%p type=%s\n", item, __typename(item));
                    (*item)();
                    item.reset();
                }
            }
...
#pragma warning(pop)
        ...
        void initialize()
        {
            if (g_palStarted.fetch_add(1) == 0) {
                ...
                ARIASDK_LOG_ERROR("Already initialized: %d", g_palStarted.load() );
            }
        }
        ...
        void shutdown()
        {
            if (g_palStarted.fetch_sub(1) == 1) {
                ...
                ARIASDK_LOG_ERROR("Shutting down: %d", g_palStarted.load() );
            }

    ...
    void Packager::handleAddEventToPackage(EventsUploadContextPtr const& ctx, StorageRecord const& record, bool& wantMore)
    {
        try
        {
            ...
            if (m_splicer.getSizeEstimate() + record.blob.size() > ctx->maxUploadSize)
            {
                ...
                {
                    LOG_TRACE("Maximum upload size %u bytes exceeded, not adding the next event (ID %s, size %u bytes)", ctx->maxUploadSize, record.id.c_str(), static_cast<unsigned>(record.blob.size()));
                    return;
                }
                else
                {
                    LOG_INFO("Maximum upload size %u bytes exceeded by the first event", ctx->maxUploadSize);
                }
            }
            ...
            if (ctx->latency == EventLatency_Unspecified)
            {
                ...
                LOG_TRACE("The highest latency found was %d (%s)", ctx->latency, latencyToStr(ctx->latency));
            }

            LOG_TRACE("Adding event %s:%s, size %u bytes", tenantTokenToId(record.tenantToken).c_str(), record.id.c_str(), static_cast<unsigned>(record.blob.size()));

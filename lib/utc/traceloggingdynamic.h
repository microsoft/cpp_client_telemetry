//depot/rs_prerelease/publicint/onecoreuapbase/inc/traceloggingdynamic.h#1 - branch change 7433535 (text)
/* ++
...
#pragma region Public interface
    ...
    - Macro:    TLD_EVENT_SET_INFORMATION
    ...
    - When the API asks for a byte-buffer, you can use std::vector<BYTE> or
      ...
      the buffer.
    - TraceLogging combines the Id and Version fields of EVENT_DESCRIPTOR to
      form a 24-bit event Identity. This Identity can improve event analysis,
      but it is not required. The event Identity 0 indicates that the event
      does not have an assigned Identity. If you don't want to manage event
      Identity, set it to 0, i.e. set both Id and Version to 0. If you do
      assign Identity to your events, the 24-bit Identity must be non-zero,
      and there should be a one-to-one mapping between {REGHANDLE + event.Id +
      event.Version} and {event metadata}. In other words, for a particular
      ETW provider lifetime (the scope of the REGHANDLE), any event with a
      given non-zero Id+Version combination must always have exactly the same
      event metadata.
    ...
    */
    ...
    Derives from EVENT_DESCRIPTOR. Adds convenient constructor, Reset, and
    Identity members. Contains an event's Id/Level/Opcode/Task/Keyword
    settings. Use of this class is optional - you can use this class if you
    want to use the constructor, Reset, or Identity methods, or you can use
...
#if (WINVER >= _WIN32_WINNT_WIN7) && WINAPI_FAMILY_PARTITION(WINAPI_PARTITION_DESKTOP)
    ...
    */
    ...
    adjusted by setting the TLD_EVENT_SET_INFORMATION and/or
    TLD_HAVE_EVENT_SET_INFORMATION macros (described below).
    */
    ...
    */
    ...
    /*
    Configuration macros for controlling the behavior of SetInformation:
    - TLD_HAVE_EVENT_SET_INFORMATION
    - TLD_EVENT_SET_INFORMATION
    ...
    When neither TLD_ macro is set and WINVER < 0x0602:
    ...
    If you set TLD_EVENT_SET_INFORMATION:
    SetInformation directly invokes TLD_EVENT_SET_INFORMATION(...) and returns
    whatever that returns.

    If you set TLD_HAVE_EVENT_SET_INFORMATION without setting TLD_EVENT_SET_INFORMATION:
    - If TLD_HAVE_EVENT_SET_INFORMATION == 0, SetInformation always returns an
      error.
    - If TLD_HAVE_EVENT_SET_INFORMATION == 1, SetInformation uses the WINVER >= 0x0602
      behavior described above (directly calls EventSetInformation).
    - If TLD_HAVE_EVENT_SET_INFORMATION == 2, SetInformation uses the WINVER < 0x0602
      behavior described above (locates EventSetInformation via GetProcAddress).
    */
#ifndef   TLD_EVENT_SET_INFORMATION
  #define TLD_EVENT_SET_INFORMATION ::EventSetInformation
    ...
    #else
      #define TLD_HAVE_EVENT_SET_INFORMATION 1 // Directly invoke TLD_EVENT_SET_INFORMATION(...)
    #endif
  #endif
#elif !defined(TLD_HAVE_EVENT_SET_INFORMATION)
  #define TLD_HAVE_EVENT_SET_INFORMATION 1 // Directly invoke TLD_EVENT_SET_INFORMATION(...)
#endif
...
#pragma region Enumerations
    ...
    enum Type : UINT16
    {
        ...
        */
        ...
        /*
        Encoding assumes nul-terminated CHAR8 string.
        Formatting treats as MBCS JSON.
        */
...
#pragma region EventDescriptor
    ...
    - If you are not assigning unique Ids for your events, set both Id and
      Version to 0. If you are assigning unique Ids for your events, treat
      Id + Version as a single 24-bit field for event identity. Identity
      should be based on event metadata and should be scoped to the lifetime
      of the provider REGHANDLE. In other words, in a particular provider
      session, every event with Id=X and Version=Y must have identical
      event metadata, unless X=0 and Y=0.
    */
    struct EventDescriptor
        : EVENT_DESCRIPTOR
    {
        ...
        /*
        Returns the 24-bit event identity, composed of Id + Version.
        */
        ...
        /*
        Sets the 24-bit event identity, composed of Id + Version.
        */
        void Identity(UINT32 identity)
        {
            Id = static_cast<USHORT>(identity);
            Version = static_cast<UCHAR>(identity >> 16);
        }
    };
        ...
        _In_opt_ void* pEnableCallbackContext)
    {
        ...
        if (status == 0)
        {
            ...
            status = tld::SetInformation(
                hProvider,
                static_cast<_EVENT_INFO_CLASS>(2), // EventProviderSetTraits
...
#pragma warning(suppress: 6387) // It's ok for pvInformation to be null if cbInformation is 0.
        status = TLD_EVENT_SET_INFORMATION(
            hProvider,
            informationClass,
            pvInformation,
            cbInformation);
...
#pragma region EventBuilder
    ...
    public:
        ...
        */
        ...
        Compatible with: TypeBinary, TypeIPv6, TypeSocketAddress.
        */
        void AddBinary(_In_reads_bytes_(cb) void const* pbData, UINT16 cbData)
        {
...
#pragma region Basic properties
        ...
        /*
        Note: identity is a 24-bit value, containing descriptor.Id + descriptor.Version.
        */
        UINT32 Identity() const
        {
            return m_descriptor.Identity();
        }
        ...
        */
        void Identity(UINT32 identity)
        {
            m_descriptor.Identity(identity);
        }

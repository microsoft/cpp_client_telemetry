...
struct RuntimeConfigTests_ECS : public Test
{
    ...
    ARIASDK_NS::RuntimeConfig_Default defaultConfig;
...
{
    ARIASDK_NS::RuntimeConfig_ECS config(ecsClientMock);
    ...
    EXPECT_THAT(config.GetEventPriority(), ARIASDK_NS::EventPriority_Unspecified);
}
...
TEST_F(RuntimeConfigTests_ECS, GetDefaultEventPriorityWithEmptyMap)
{
    ...
    config.SetDefaultConfig(defaultConfig);
    EXPECT_THAT(config.GetEventPriority(), ARIASDK_NS::EventPriority_Unspecified);
    EXPECT_THAT(config.GetEventPriority("1", "3"), ARIASDK_NS::EventPriority_Unspecified);
}
...
TEST_F(RuntimeConfigTests_ECS, GetEventPriorityReturnsValueFromInitialMap)
{
    ...
    ARIASDK_NS::RuntimeConfig_ECS::EventPriorityMap initialMap = {
        { ARIASDK_NS::RuntimeConfig_ECS::EventKey{"",             ""         }, ARIASDK_NS::EventPriority_Off       },
        { ARIASDK_NS::RuntimeConfig_ECS::EventKey{customTenantId, ""         }, ARIASDK_NS::EventPriority_Low       },
        { ARIASDK_NS::RuntimeConfig_ECS::EventKey{customTenantId, customEvent}, ARIASDK_NS::EventPriority_Immediate },
    };
    ARIASDK_NS::RuntimeConfig_ECS config(ecsClientMock, initialMap);
    expectInitialEcsConfig(0);
    config.SetDefaultConfig(defaultConfig);
    EXPECT_THAT(config.GetEventPriority(),                            ARIASDK_NS::EventPriority_Off);
    EXPECT_THAT(config.GetEventPriority("1",            "3"),         ARIASDK_NS::EventPriority_Off);
    EXPECT_THAT(config.GetEventPriority(customTenantId),              ARIASDK_NS::EventPriority_Low);
    EXPECT_THAT(config.GetEventPriority(customTenantId, "2"),         ARIASDK_NS::EventPriority_Low);
    EXPECT_THAT(config.GetEventPriority(customTenantId, "3"),         ARIASDK_NS::EventPriority_Low);
    EXPECT_THAT(config.GetEventPriority(customTenantId, customEvent), ARIASDK_NS::EventPriority_Immediate);
}
...
TEST_F(RuntimeConfigTests_ECS, GetEventPriorityReturnsValueFromEcsMap)
{
    ...
    EXPECT_CALL(*ecsConfigMock, GetSettingAsInt(StrEq("SCT"), StrEq("Priority/tenantId2/Default"), _))
        .WillOnce(Return(ARIASDK_NS::EventPriority_High));
    ...
    EXPECT_CALL(*ecsConfigMock, GetSettingAsInt(StrEq("SCT"), StrEq("Priority/tenantId2/Events/event3"), _))
        .WillOnce(Return(ARIASDK_NS::EventPriority_Normal));
    ...
    EXPECT_THAT(config.GetEventPriority(),                            ARIASDK_NS::EventPriority_Unspecified);
    EXPECT_THAT(config.GetEventPriority("tenantId1", ""),              ARIASDK_NS::EventPriority_Off);
    EXPECT_THAT(config.GetEventPriority("tenantId2", "event1"),        ARIASDK_NS::EventPriority_Off);
    EXPECT_THAT(config.GetEventPriority("tenantId2", "unknown event"), ARIASDK_NS::EventPriority_High);
    EXPECT_THAT(config.GetEventPriority("tenantId2", "event1"),        ARIASDK_NS::EventPriority_Off);
    EXPECT_THAT(config.GetEventPriority("tenantId2", "event2"),        ARIASDK_NS::EventPriority_Unspecified);
    EXPECT_THAT(config.GetEventPriority("tenantId2", "event3"),        ARIASDK_NS::EventPriority_Normal);
    EXPECT_THAT(config.GetEventPriority("tenantId2", "event4"),        ARIASDK_NS::EventPriority_Immediate);
}

TEST_F(RuntimeConfigTests_ECS, GetEventPriorityRespectsEcsWithHighestPriority)
{
    ARIASDK_NS::RuntimeConfig_ECS::EventPriorityMap initialMap = {
        { ARIASDK_NS::RuntimeConfig_ECS::EventKey{"tenantId1", "event1"}, ARIASDK_NS::EventPriority_Low    },
        { ARIASDK_NS::RuntimeConfig_ECS::EventKey{"tenantId2", "event2"}, ARIASDK_NS::EventPriority_Normal }
    };
    expectInitialEcsConfig(0);
    ARIASDK_NS::RuntimeConfig_ECS config(ecsClientMock, initialMap);
    config.SetDefaultConfig(defaultConfig);

    config.SetEventPriority("tenantId1", "event1", ARIASDK_NS::EventPriority_High);
    config.SetEventPriority("tenantId2", "",       ARIASDK_NS::EventPriority_High);
    ...
    EXPECT_CALL(*ecsConfigMock, GetSettingAsInt(StrEq("SCT"), StrEq("Priority/tenantId1/Default"), _))
        .WillOnce(Return(ARIASDK_NS::EventPriority_Immediate));
    ...
    EXPECT_THAT(config.GetEventPriority("tenantId1", "event1"), ARIASDK_NS::EventPriority_Immediate);
    EXPECT_THAT(config.GetEventPriority("tenantId2", "event2"), ARIASDK_NS::EventPriority_Normal);
}
...
TEST_F(RuntimeConfigTests_ECS, GetEventPriorityRespectsInitialConfigWithHigherPriority)
{
    ...
    EXPECT_THAT(config.GetEventPriority("tenantId1", "event1"), ARIASDK_NS::EventPriority_Low);
    EXPECT_THAT(config.GetEventPriority("tenantId2", "event2"), ARIASDK_NS::EventPriority_Normal);
}
...
TEST_F(RuntimeConfigTests_ECS, SetEventPrioritySetsCustomPriorityForTheEvent)
{
    ...
    config.SetEventPriority("tenantId1", "HighPrioEvent", ARIASDK_NS::EventPriority_High);
    config.SetEventPriority("tenantId1", "LowPrioEvent",  ARIASDK_NS::EventPriority_Low);
    config.SetEventPriority("tenantId1", "",              ARIASDK_NS::EventPriority_Normal);
    config.SetEventPriority("",          "",              ARIASDK_NS::EventPriority_Immediate);

    EXPECT_THAT(config.GetEventPriority("tenantId1",        "HighPrioEvent"), ARIASDK_NS::EventPriority_High);
    EXPECT_THAT(config.GetEventPriority("tenantId1",        "LowPrioEvent"),  ARIASDK_NS::EventPriority_Low);
    EXPECT_THAT(config.GetEventPriority("tenantId1",        ""),              ARIASDK_NS::EventPriority_Normal);
    EXPECT_THAT(config.GetEventPriority("tenantId1",        "AnyOtherEvent"), ARIASDK_NS::EventPriority_Normal);
    EXPECT_THAT(config.GetEventPriority("",                 ""),              ARIASDK_NS::EventPriority_Immediate);
    EXPECT_THAT(config.GetEventPriority("AnyOtherTenantId", "AnyOtherEvent"), ARIASDK_NS::EventPriority_Immediate);
}

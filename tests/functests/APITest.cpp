...
TEST(APITest, LogManager_KilledEventsAreDropped)
{
    ...
    configuration[CFG_MAP_METASTATS_CONFIG]["interval"] = 0; // avoid sending stats for this test
...
TEST(APITest, Pii_DROP_Test)
{
    ...
    config[CFG_MAP_METASTATS_CONFIG]["interval"] = 0;        // avoid sending stats for this test
...
TEST(APITest, LogConfiguration_MsRoot_Check)
{
    ...
    for (const auto& params : testParams)
    {
        ...
        auto& config = LogManager::GetLogConfiguration();
        config[CFG_MAP_METASTATS_CONFIG]["interval"] = 0;  // avoid sending stats for this test, just customer events
